...
def _records():
    return [
        sidecar_record_from_aflow_fields({
            "auid": "aflow:0000000000000aaa", "geometry": "[4,4,4,90,90,90]",
            "species": ["Si"], "positions_fractional": [[0, 0, 0]],
        }),
        sidecar_record_from_aflow_fields({
            "auid": "aflow:0000000000000bbb", "geometry": "[5,5,5,90,90,90]",
...
def test_sidecar_material_id_maps_to_aflow_prefix():
    rec = _records()[0]
    assert rec["material_id"] == "aflow-0000000000000aaa"
    # Exact AUID becomes a hyphenated material ID, matching HDF5.
...
def test_missing_fields_breakdown_counts_per_field():
    ...
    rec = sidecar_record_from_aflow_fields({"auid": "aflow:0000000000000fff", "geometry": "[4,4,4,90,90,90]"})
