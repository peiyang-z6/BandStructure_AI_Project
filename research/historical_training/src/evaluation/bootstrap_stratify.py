...
def group_bootstrap_ci(
    ...
    if len(unique_groups) < 2:
        return {"status": "insufficient_groups", "averaging": averaging, "resampling_unit": "spacegroup",
                "point_estimate": float(np.mean(y_true == y_pred)) if len(y_true) else None,
                "ci_low": None, "ci_high": None, "bootstrap_mean": None, "bootstrap_std": None,
                "n_boot": 0, "requested_n_boot": n_boot, "n_groups": len(unique_groups), "n_samples": len(y_true)}
    by_group = {g: np.flatnonzero(groups == g) for g in unique_groups}
    group_acc = {g: float(np.mean(y_true[idx] == y_pred[idx])) for g, idx in by_group.items()}
    point = (float(np.mean(list(group_acc.values()))) if averaging == "group_macro"
             else float(np.mean(y_true == y_pred)))
    stats = []
    for _ in range(n_boot):
        chosen = rng.choice(unique_groups, size=len(unique_groups), replace=True)
        if averaging == "group_macro":
            stats.append(float(np.mean([group_acc[g] for g in chosen])))
        else:
...
def source_catalog(sample: Dict[str, Any]) -> str:
    ...
    return sample.get("source_catalog") or catalog
...
def build_error_stratification(
    ...
    strata_specs = [
        ("provider_type", lambda s: s.get("provider_global_electronic_type", s.get("gap_type", "unknown"))),
    ...
    agreement = [{0: "agree", 1: "conflict"}.get(s.get("line_global_disagreement"), "unknown")
                 for s in samples]
    out = {"task": task_name,
           "agreement": stratify_errors(y_true, y_pred, agreement, stratum_name="agreement"),
           "agreement_cross": {}}
    for name, fn in strata_specs:
        values = [fn(s) for s in samples]
        out[name] = stratify_errors(y_true, y_pred, values, stratum_name=name)
        out["agreement_cross"][name] = stratify_errors(
            y_true, y_pred, [f"{a}|{v}" for a, v in zip(agreement, values)],
            stratum_name=f"agreement_x_{name}")
    return out
...
def summarize_task(
    ...
    if groups is not None:
        ...
        summary["group_macro_bootstrap"] = group_bootstrap_ci(
            y_true, y_pred, groups, n_boot=n_boot, random_state=random_state, averaging="group_macro"
        )
        summary["group_macro_accuracy"] = summary["group_macro_bootstrap"]["point_estimate"]
    return summary
